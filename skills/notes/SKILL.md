...
`import_doc` 和 `append_doc` 的 `content` 字段仅支持 Markdown，**不支持本地图片**。
...
ima_api "openapi/note/v1/get_doc_content" '{"note_id": "目标 note_id", "target_content_format": 0}'
```

### 列出自己有哪些笔记

直接使用`list_note`，拉取全部笔记列表，直到符合用户要求（如：用户要求全部笔记，则到 `is_end=true` 时停止；用户要求近 30 天的笔记，则根据`modify_time`与当前时间判断停止）
...
ima_api "openapi/note/v1/list_note" '{"folder_id": "目标 folder_id", "cursor": "", "limit": 20}'
```
...
```bash
ima_api "openapi/note/v1/append_doc" '{"note_id": "笔记 ID", "content_format": 1, "content": "\n## 补充内容\n\n追加的文本"}'
```
